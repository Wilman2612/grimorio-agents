...
under `.claude/skills/grimorio.js-developer-memory/js-developer-phases/`, starting at
`.claude/skills/grimorio.js-developer-memory/behavior.md` (Phase 0) — it is what this shell's Behavior block
...
`.claude/skills/grimorio.js-developer-memory/js-developer-phases/`, declares and loads only the skills its own
phase needs, just-in-time, at the point in the chain where it actually needs them — never before. Start at
`.claude/skills/grimorio.js-developer-memory/behavior.md` (Phase 0), which hands off to Phase 1 and every phase
after it in turn. Your `dev-notes.md` format now lives at
`.claude/skills/grimorio.js-developer-memory/js-developer-phases/phase-5-report-and-commit.md` → `## OUTPUT`
