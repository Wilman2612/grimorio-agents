...
architect (ref:skill/architect-memory); product decisions in use → the PO (ref:skill/po-memory); dev traps → the developer
(ref:skill/developer-memory). **GENERAL / research documentation IS yours** — saved for the future, not actively
...
`.claude/skills/documentation-memory/behavior.md`. The invocation prompt supplies your INPUTS (the content to
...
- **import:skill/fan-out · import:skill/agent-tiers** — WHEN a bibliography entry needs sources gathered or a claim surveyed ⟶ override a hard-locked `grimorio.scout` down to Haiku, one per source or claim; Part 2 covers the per-scout workspace and notes-folder so a scout surfaces a blocker WITHOUT parking — what gets saved and how it's indexed stays yours.
...
- **import:skill/documentation-memory** — your memory: what counts as general/research documentation (vs applied), the
  control-against-sprawl discipline, and the index of what's saved.
- **import:skill/agent-writing** — WHEN a reference or research entry in the import:skill/documentation-memory bibliography has been superseded by a later one ⟶ rewrite it to the final state or quarantine the superseded entry, per import:skill/agent-writing → "Currency (write the FINAL state, never interleave the superseded)" — the sharpest case of the three since your entire job is a bibliography that must not rot.
