---
name: grimorio.report-design
description: "How to present a report / design / change-proposal / research finding to a HUMAN reviewer (esp. the CEO) DIGESTIBLY. The structure — verdict-first → 3-5 theme table → optional diagram → collapsible detail — AND the load-bearing rule that MECHANICS MUST BE SHOWN VISUALLY, not just named. WHEN the SUBJECT itself is a complex multi-part system, not just the change being reported, the companion ./project.complex-systems.md extends this into one-diagram-PER-VIEW (information-type taxonomy, form-per-concept table, visual-language discipline, per-view ship gate). Load before writing ANY report, design doc, or change summary a human will review. Grounded in a convergent cross-industry standard (the research below), not invented."
---
...
## When the SUBJECT is a complex system — "one diagram" means one diagram PER VIEW

The "One diagram" line above is scoped to ONE report about ONE change.

**WHEN the report's SUBJECT is itself a multi-part system (real structure, a real flow across parts, a real feedback loop) ⟶ treat the unit as the VIEW, not the document: an overview plus one focused view per sub-mechanism, never one diagram for the whole thing.** Reading "one diagram" as "one diagram for the whole system" is exactly how a fused mega-graph gets built — several information-types crammed into one picture, redundant with tables and sequence diagrams already sitting beside it.
-> deeper: ./project.complex-systems.md — the information-type taxonomy, a form-per-concept decision table, the leveling and visual-language rules, a worked example, and the per-view ship gate. Load it before diagramming any subject with real multi-part structure.
...
> sub-problem is real, and who fixed each constraint you are about to defend, is ref:skill/grimorio.reasoning-principles →
...
2. **Take each one to the BASES.** Vision, product memory, the mechanics ledger, the features list, the
   ...
   rule, a question escalated to the CEO about ladder budgets was already ruled in `this project's own product-vision record`, and
