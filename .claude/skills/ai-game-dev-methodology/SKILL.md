...
- **Critics are GAMEABLE.** A builder iterating against a critic learns the critic's PROXY, not the goal — and
  ...
  - **Critic tier ≥ generator tier** (a rule ref:skill/agent-tiers#critic-integrity--the-one-tiering-rule-you-cannot-cheap-out-on did NOT have — added there).
...
- **Rule:** an autonomous / unattended run MUST carry a hard failsafe — a step, token, and wall-clock/dollar
  ...
  unbroken context). Folded into ref:skill/flow-delegation#the-two-operating-modes--normal-and-autonomous (the top-level autonomous loop, not just delegates).
...
- Extend the reference-first rule (ref:skill/agent-selection#reference-first-for-visual--aesthetic-deliverables-weak-domain-compensation-hard-rule) explicitly to mechanics/systems design.
