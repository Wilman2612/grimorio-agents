---
name: po-memory
description: "Semantic memory for the PO agent. SKILL.md (general) = universal Product Owner principles and the product-mode classification. For this project's product context (project) read ./project.md; for feature-flag status (code) read ./features-status.md."
---
...
- **A signed/ACCEPTED decision's content must live in a repo-tracked file, never only a `tmp/` pointer**
  ...
  ACCEPTED/SIGNED/"source of record" in `./vision.md` or `./project.md`, verify with `git ls-files <path>` that
  the cited path is tracked. If it isn't, migrate the substance into `po-memory/docs/` first, then cite that.
...
-> Feature-flag status (what's built, disabled, planned): read ./features-status.md
