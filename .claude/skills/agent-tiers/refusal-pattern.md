...
This file is a topic-reference companion to ref:skill/agent-tiers — general knowledge, the same status as its
`./reference.md` sibling, **not** a behavior file, because no single agent owns this pattern. Two more
...
(ref:skill/flow-delegation/delegate-behavior.md), and the executor developers' missing-plan refusal
(ref:skill/developer-memory/build-protocol.md). WHEN you land here first, read this as one of several places
...
ref:skill/developer-memory/build-protocol.md#missing-plan-refusal--this-developers-own-instantiation-of-the-identity-refusal-pattern's
...
ref:skill/grimorio-conduct/main-loop-only.md already rests its rule 6 on: the caller planned without the
...
ref:skill/agent-tiers#haiku-the-volume-tier--plan-on-sonnetopus-execute-on-haiku-review-on-sonnet — Haiku is
...
ref:skill/agent-tiers#it-does-not-reliably-recognise-its-own-skills--name-them-explicitly. **State this plainly:
