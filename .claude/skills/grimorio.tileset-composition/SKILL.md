---
name: grimorio.tileset-composition
...
1. **Cut ON the design grid, never off it.** Find the sheet's real grid (it divides the sheet's pixel dimensions
   evenly); an off-grid/sub-pixel cut re-introduces seams that don't exist in the source. -> deeper: `./project.cutting.md`.
2. **Verify by IMAGE, not by prose.** The seam test (tile a candidate cell into a 3×3 block and inspect the
   ...
   doc (including this one) is a hypothesis until confirmed against the reference image. -> deeper: `./project.workflow.md`.
3. **CENTER vs EDGE/FADE cells play different roles — never interchange them.** The CENTER cell must
   ...
   mid-run is the single most common cut bug (a foreign tile breaking a straight run). -> deeper: `./project.cutting.md`.
4. **Autotiling composes transitions from a selector, never by hand-placing every combination.** Reuse the
   ...
   selector. -> deeper: `./project.autotiling.md`.
5. **In a ¾ top-down camera, a tall face is drawn ONLY on the side facing the camera — south.** This is the one
   ...
   pixel-verified against the specific asset. -> deeper: `./project.elevation.md`.
6. **Layering/z-order is what makes reused pieces read as different heights or roles — not new art.** The same
   ...
   floor," "behind the player," or "in front of the wall" without drawing anything new. -> deeper: `./project.layering.md`.
7. **A unified palette is the #1 coherence lever** — more than resolution, animation, or tile complexity, for
   making separately-sourced or separately-drawn pieces read as one place. -> deeper: `./project.coherence.md`.

## Map of the reference files (load only what the task touches)

| File | Covers | Load when |
|---|---|---|
| `./project.cutting.md` | Grid detection, the seam test, 9-slice/9-patch anatomy, CENTER vs EDGE/FADE roles | Cutting a sheet into tiles for the first time, or a seam/edge-cell bug |
| `./project.autotiling.md` | Bitmask/blob-47, corner-Wang, multi-terrain priority + routing | Building or reviewing terrain transition logic |
| `./project.workflow.md` | The reference-first breakdown method: grid → geometry → seam-test → classify cell ROLE → symmetric/unique → verify-by-image loop | Reading/analyzing an unfamiliar tileset before building anything with it |
| `./project.layering.md` | Layer order, draw-priority tricks, passability-vs-draw decoupling, the "second floor from reused pieces" family of tricks, sprite-stacking | Compositing multiple pieces/layers into one read (height, occlusion, floors) |
| `./project.elevation.md` | The ¾ cliff/elevation model: the universal south-face rule + the engine-dependent stacking families | Building or reviewing any raised-terrain/cliff brush |
| `./project.coherence.md` | Decompose+reorient, center-variation without asset explosion, the unified-palette lever | Making disparate or separately-sourced pieces read as one place |
...
- **Building/fixing a terrain brush or elevation**: apply `./project.cutting.md` (cut on-grid, center-vs-edge) →
  `./project.autotiling.md` (reuse the autotile selector) → `./project.elevation.md` (south-only face, then pixel-verify this
  tileset's own stacking) → `./project.layering.md` (draw order, occlusion) → `./project.coherence.md` (palette + reorient, never
  smooth).
- **Reading/analyzing an unfamiliar tileset before building with it**: start at `./project.workflow.md` — it is the
...
- **Reviewing a render** (e.g. via a per-brush visual critic): the concrete cut/composition parameters to check
...
this skill's *researched* craft): `./project.field-lessons.md`. Read it before building any brush; append to it when a build
...
`grimorio.documentation-memory/docs/` (see that skill's index for the exact filename).
