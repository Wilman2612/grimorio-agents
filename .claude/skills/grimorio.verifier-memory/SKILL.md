---
name: grimorio.verifier-memory
description: "Semantic memory for the manual-verifier agent. SKILL.md (general) = universal visual-verification principles, the state-machine coverage protocol, scenario planning, error-capture rules, the visual checklist, and the BLOCKED-hardware list. For this project's local setup (project/code) read ./project.md and grimorio.verifier-memory/project.local-setup.md."
---
...
-> This project's local servers, test accounts, seed data: read ./project.md and this project's own local-setup notes
-> The `verification-report.md` output format: ./behavior.md → `## OUTPUT`. Screenshot organization: ref:skill/grimorio.feature-workflow#artifact-directory-structure skill → Artifact Directory Structure.
