...
-> ref:skill/fan-out#the-caller-not-the-callee-owns-the-split-hard-rule-ceo-2026-08-10 for the general mechanism
this instantiates.
...
-> ref:skill/flow-delegation#part-1--the-flow-brief-template-how-you-raise-the-delegate (item 4).
...
(ref:skill/flow-delegation#part-1--the-flow-brief-template-how-you-raise-the-delegate, item 3). THIS is the
...
self-only correction (CRITIC, ICLR'24, cited in ref:skill/flow-delegation) in disguise: it stays subject to
...
-> ref:skill/grimorio-conduct/main-loop-only.md for the top-level session's own binding of this rule when
reporting to the CEO
-> ref:skill/agent-writing/system-keeper-behavior.md for what grimorio.system-keeper owes on its own close when
...
4. **Only then decide** what changes, what you ask about, and what genuinely cannot move.
...
an objection — he has explicitly invited that (ref:skill/po-memory/decisions.md decision 36). A private
...
5. **OPEN** — what you could not decide and what you assumed instead, so a reader can attack the
   assumption rather than the result.
...
-> import:skill/fan-out#emit-the-loop-graph-before-you-spawn-or-write-hard-rule-ceo-2026-08-08
...
-> The machine you run once decomposition is done — the loop, the graph, the retry/FINDING exit, the
   probe: ref:skill/loop-and-graph.
...
- **A test ⟶ MUTATE the code it guards and watch it go red.** Green-only is not evidence.
  (ref:skill/qa-memory#the-break-proof--a-test-you-have-not-seen-fail-is-not-evidence)
