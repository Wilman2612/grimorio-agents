...
4. Restore the code, re-run, confirm green **and** confirm the tree is clean (`git status`) — a mutation left
   behind is worse than no test.

State in your report which tests were break-proofed and how. A test whose failure nobody has observed is a
claim, not a probe — the same rule cite:repo/objectives/harness.md GATE 5 applies to gates.
...
- **This tier is the mandatory happy-path floor** — every feature's happy path is proven here before any
  ...
  (ref:skill/qa-memory#where-e2e-sits--the-happy-path-floor-not-an-on-demand-layer-ceo-2026-08-13).
...
-> ref:skill/reasoning-principles → "MEASURING IS NOT PROVING".
