...
**import:skill/po-memory/features-status.md#what-is-green-stated-at-the-width-the-code-actually-delivers-re-verified-2026-07-30 is the record of what is ALREADY BUILT.** Read it before you
...
(ref:skill/documentation-memory doc 59). A survey or design brief that does not say what already exists is the tell that
...
> anti-bucket defect the objective gate already exists to catch (ref:repo/objectives/harness.md#hard-invariants--these-are-gates-not-preferences) — the fix is to SPLIT
> the branch, never to skip the review.** A branch whose diff no longer states one sentence without "and also" is
> already past the point where "once at the end" is doing what it was written to do.
...
### Render conventions → `grimorio.conventions-critic`

A game RENDER (not a map, not one brush) is gated by agent:grimorio.conventions-critic: it scores the render
...
passed. Its trigger otherwise lives only inside ref:skill/game-development/conventions.md, one hop from where you
decide which critic to spawn.

Do not confuse it with the two map critics: `map-aesthetic-critic` judges a MAP's composition, `brush-critic`
judges ONE terrain brush. This one judges a running render against game conventions.
...
independence, not capability — lives at ref:skill/flow-delegation#independence-not-capability--why-you-raise-a-delegate-ceo-ruling-2026-08-12.
...
spawn, and then GUARD it: watch its milestones, not every file. -> ref:skill/flow-delegation#part-1--the-flow-brief-template-how-you-raise-the-delegate for the brief template and
...
(ref:skill/agent-writing#the-levels--behavior--general--project--code's four-level split). **If you catch yourself writing "if you are the child…" in shared context,
...
- a **WEB app** change (apps/web + its backend) → agent:grimorio.web-architect (reads the PO brief; owns the
  frontend↔backend/DAL contract, OWASP).
- a **GAME** change (the war-sim OR its replay render) → agent:grimorio.game-architect. This is ONE agent that runs
...
- a change to **grimorio itself** — its own agents, skills, hooks, or rules — → agent:grimorio.system-keeper,
  ...
  authors what it decides — `grimorio.prompt-writer` does. -> ref:skill/agent-writing/system-keeper-behavior.md#the-five-phases--analysis-verification-planning-execution-review
  for how it decides and coordinates — not restated here.
...
| "Should we build / buy / borrow / reuse X?" (with OPEX) | agent:grimorio.solution-architect | Decides reuse > borrow > buy > build; owns the stack inventory + the ref:skill/fan-out#part-1--decompose-spawn-in-parallel-synthesize methodology. |
...
> objective; add only context; author no scope of your own** — the two binary checks in ref:skill/agent-writing#2-identity-paragraph
...
-> full rule, the in-repo proof, and the construction-level defense (the personality/behavior split): ref:skill/agent-writing#2-identity-paragraph skill
...
|---|---|---|
| A confirmed **product** decision/priority (offering, economy, categories) | agent:grimorio.po | ref:skill/po-memory |
| A non-obvious **WEB architecture** decision (structure, why, how pieces combine) | agent:grimorio.web-architect | ref:skill/architect-memory |
| A non-obvious **GAME** design/architecture decision (a mechanic + how it lands in sim/render code) | agent:grimorio.game-architect | ref:skill/game-design |
| A non-obvious **dev** gotcha (hard library, trap) | agent:grimorio.js-developer | ref:skill/developer-memory/traps.md |
| A finished **research/investigation**, or reference/theory to keep for later | agent:grimorio.documentation | ref:skill/documentation-memory |
...
1. **Gather concrete visual REFERENCES and distil the target look FIRST, before building.** Never build toward a
   ...
   to supply it.
2. **Judge against the reference through the ADVERSARIAL visual critic** (agent:grimorio.map-aesthetic-critic,
   agent:grimorio.brush-critic, or a UX critic) — **never** the builder's self-report.
...
-> The full post-mortem of this failure mode: ref:skill/documentation-memory/docs/44-terrain-render-postmortem-forensic-record.md#0-method-and-its-limits-read-before-trusting-anything-below
-> How to run visual work by parts (per-brush isolated maps, per-part screenshots): ref:skill/map-design skill
...
|---|---|---|---|
| A design/decision is about to be finalized and nobody has challenged it | agent:grimorio.entropy | per ref:skill/agent-tiers#the-orchestration-cascade-cost-discipline--enforce-it | Divergent ranked blind-spots. Proactive — fires before trouble. |
| ONE concrete technical blocker: failing build, infra dead-end, missing capability, under-specified fork | agent:grimorio.unblocker | per ref:skill/agent-tiers#the-orchestration-cascade-cost-discipline--enforce-it | A verified way through, resolved autonomously if reversible+low-impact, else a decision-ready escalation. |
...
| The assistant system itself is failing, drifting, or undesigned — a grimorio agent/skill/hook misbehaving, a rule that fires nowhere, a role nothing owns | agent:grimorio.system-keeper | **Opus** — the coordinator/orchestrator default, per ref:skill/agent-tiers#every-agent-declares-its-own-default--omit-model-ceo-fix-2026-07-29-enforced-here-2026-07-30 | A decided change, already placed, verified, and gated — nothing further to route. |
...
(ref:skill/agent-selection#hard-rules-of-invocation-mirrored-as-triggers-in-claudemd--agent-selection), applied
to one more agent.

**`adviser` is the ONE agent at FABLE** (CEO, 2026-07-29) — a deliberate ref:skill/agent-tiers#the-one-standing-fable-exception-plus-a-second-distress-signal-at-opus-mandated-not-judgment-calls EXCEPTION: the
...
The CEO asked for the loop-vs-graph distinction (ref:skill/documentation-memory doc 58) to be re-analysed as a
...
- **Work that matches a known shape** — a feature, a bug, a refactor, anything whose deliverable needs a gate —
  ...
  practiced and measured in this repo — the method is ref:skill/fan-out#split-planning-from-execution--a-temporal-axis-for-pieces-that-only-collide-at-write-time-ceo-2026-08-03
  ("Split PLANNING from EXECUTION"), and the measurement is
  ref:skill/fan-out/measured-runs.md#measured-the-same-fan-out-task-three-stages-three-outcomes-ceo-2026-08-04
  ...
  **What CARRIES an authored plan is a FILE, per piece** —
  ...
  paragraph") and in ref:skill/fan-out (each child owns `tmp/<child-id>/`); the reason is
  ref:repo/.claude/GRIMORIO-CHAIN.md#7-the-loss-map--every-chain-and-exactly-where-it-breaks's Loss 3 — a
  paraphrase degrades at every hop, a path does not. **Per-agent EMISSION of that plan now has a standing rule
  and a home, not just a practiced method:**
  ref:skill/fan-out#emit-the-loop-graph-before-you-spawn-or-write-hard-rule-ceo-2026-08-08 requires every
...
| Greenfield / full feature (short form) | architect → developer → developer → developer → agent:grimorio.qa → agent:grimorio.code-reviewer (before merge) | CEO, quoted below; full artifact-handoff version: `import:skill/feature-workflow#routing-rules`, "Feature" row |
| Bug fix | security triage (text-only) → architect check (if hard/cross-service) → developer diagnoses → architect validates (if step 3 says hard/multi-layer) → manual-verifier confirms the bug → developer fixes → qa regression → manual-verifier confirms the fix | `import:skill/feature-workflow#bug-triage-progressive-escalation` |
| Refactor | architect → developer → qa → code-reviewer | `import:skill/feature-workflow#routing-rules`, "Refactor" row |
...
parallel, the adversarial cluster fanned out — the sequence and artifact contracts live in the ref:skill/feature-workflow#artifact-directory-structure
