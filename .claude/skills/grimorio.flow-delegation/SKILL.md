...
in ref:skill/grimorio.fan-out#part-2--stay-reachable-report-back-without-parking's Part 2 ("Stay reachable"); this skill adds the OBJECTIVE and the GUARD on top of it.
...
> operationalizes ref:skill/grimorio.agent-writing#2-identity-paragraph's PRINCIPAL-INTENT FIDELITY.
...
*(Moved out of `CLAUDE.md` 2026-07-30. The full CEO ruling: this project's own research bibliography.)*
...
- **A one-shot mechanical patch: the LIGHTWEIGHT form.** Do not put a 6-part flow-brief + saved invocation +
  watcher on a Haiku one-line fetch/patch grunt — ref:skill/grimorio.fan-out#the-volume-fan-out-ladder--when-an-agent-fans-out-n-children-of-its-own-type-six-step-algorithm explicitly rejects that ceremony. Give it the
...
- **ADVERSARIAL / GATE delegates: NO completion-check list — this is a HARD carve-out.** A critic, reviewer,
  ...
  IS the "accepted-limits allowlist of what NOT to flag" that the INVOCATION-BIAS rule (ref:skill/grimorio.agent-writing#2-identity-paragraph, HARD,
...
- **The main loop: when autonomous mode is activated** — the principal assigns a long task or says he is leaving
  ...
  the checks (backlog empty) hold. -> the activation logic: this project's own research bibliography; the two modes
  themselves: this skill's own "The two operating modes — NORMAL and AUTONOMOUS" section above. `CLAUDE.md` no
  longer carries a section of its own — the chain reaches this skill through the rule to load import:skill/grimorio.flow-delegation
  ...
  > ref:skill/grimorio.ai-game-dev-methodology.
...
correction reliably DEGRADES quality** (Huang et al., ICLR 2024, `cite:arxiv/2310.01798`) — an agent grading its
own "am I done?" drifts. Flow mode
...
delegate or advance on a non-trivial objective, answer these five IN WRITING; if you can't, you don't have a flow
...
3. **REQUIREMENT-COVERAGE** — an independent, cheap evaluator holds the principal's VERBATIM request —
   Part 1 item 1's own quoted objective, never Part 0 item 1's own DONE-for-the-whole (already an authored,
   translated acceptance bar, one decomposition step removed from his own words) — plus the WRITTEN plan
   item 2 just produced, and returns a coverage map: each request clause maps to the plan item(s) delivering
   it, or UNCOVERED.

   **WHEN any request clause is UNCOVERED ⟶ STOP and re-plan before executing; never proceed with an uncovered
   clause.**

   **NEVER let the evaluator be the planner, and NEVER let it read a paraphrased or decomposed proxy for the
   request in place of the request VERBATIM.** It MUST NOT be the planner, since self-only correction reliably degrades quality (Huang et al., ICLR 2024,
   `cite:arxiv/2310.01798`, already grounding this file's own "When flow mode applies" section above; CRITIC,
   `cite:arxiv/2305.11738`, already grounding Part 1's own "Completion checks" section below), and it must read the VERBATIM request, never the decomposed version, or it inherits the exact
   dilution it exists to catch — this closes the same hole Part 2 rule 3 names in its own text: "when the
   guardian IS the main loop that authored the brief, it can't catch its OWN brief being wrong."

   **ALWAYS raise the evaluator as a FRESH, hard-locked, non-recursive agent:grimorio.scout spawn, never the
   same invocation/context as whoever authored the plan — at SONNET, never Haiku.** This evaluator IS "checking
   that rules were followed" (whether the plan followed the request), and
   ref:skill/grimorio.agent-tiers#haiku-as-the-first-option-for-executors--two-sanctioned-shapes-never-a-third-ceo-ruling-2026-08-12's
   own unconditional rule bars Haiku from any review gate regardless of what tier produced the work under
   review — no floor or tie-breaker needed. `grimorio.scout`'s own declared default is already `sonnet`, so
   raising it this way needs no `model` override. This depends on item 2's plan being a genuine WRITTEN
   ARTIFACT structured clause-by-clause, per
   ref:skill/grimorio.loop-and-graph#2c-the-plan-is-an-artifact-every-level-expands-ceo-2026-08-13, never an ad hoc
   paragraph the evaluator has to reverse-engineer.

   **WHEN the flow is a one-shot mechanical patch ⟶ this gate does not apply** — the SAME LIGHTWEIGHT form
   carve-out this file's own "When flow mode applies" section already defines binds here too, the same
   exemption the whole five-item pre-flight above already carries — restated here because this is the newest,
   least-familiar item. The expensive
   counterpart — VERIFYING what actually got BUILT against the original request, not just what got PLANNED — is
   a DEFERRED future follow-up, out of scope here (Boehm 1979; IEEE 1012, the verification-vs-validation
   distinction — the same split CHECK 3 of
   ref:skill/grimorio.system-design/design-orchestrator-phases/phase-6-converge-verify-validate.md#steps already cites
   this way).
4. **TEST PLAN** — which INTEGRATION scenario proves which sub-task's DONE (+ the whole-composition coherence check).
5. **STAGING** — the sequence + dependencies; where the gate runs.

**ONE LOOP, ONE OBJECTIVE — a loop ends where its objective ends (CEO correction, 2026-08-06).** A broad
capability needs SEVERAL loops in STAGING order (item 5 above), never one delegate run stretched across
...
-> **HOW to answer each, the practitioner method + a full WORKED EXAMPLE: `./project.flow-definition.md`.** This is the
   ...
   — the objective per ref:skill/grimorio.agent-writing#2-identity-paragraph (PRINCIPAL-INTENT, the four-elements), the checks per ref:skill/grimorio.prompt-writing-quality;
...
(this project's own research bibliography) and
...
this project's own research bibliography§3--deliberate-ignorance-how-it-is-recorded-and-whether-anything-reopens-it,
...
1. **Objective — the principal's goal, in the principal's OWN words.** One sentence, quoted and marked
   authoritative. This composes with the **PRINCIPAL-INTENT FIDELITY** rule (ref:skill/grimorio.agent-writing#2-identity-paragraph): your acceptance
...
2. **Full context — enough that the delegate could answer FOR you.** Background, prior decisions, and the shared
   ...
   - **Identify and pass the OWNING design/spec docs as MANDATORY first-reading — not just task/render context.**
     ...
     wiring + Tiny Swords assets but NOT this project's own game-design archive / `economy-mechanics.md`, so it rendered
...
3. **Completion checks — a NUMBERED, checkable list it is NOT DONE until every item holds.** Not prose, and not
   ...
   **The exercise is the BAR; the evidence-artifact and anchor-lock rules below are the FLOOR beneath it** —
   ...
   - **Each check names its EVIDENCE ARTIFACT — never the delegate's own say-so.** A check whose only proof is the
     delegate asserting "done" is not a completion check (CRITIC, `cite:arxiv/2305.11738`, doc 52 §2#9: external
     verification is REQUIRED, not "ideally" — Huang et al., ICLR 2024, `cite:arxiv/2310.01798`: self-only
     attestation reliably degrades quality — and un-anchored checks invite specification-gaming à la Krakovna
     2020 / Goodhart). Each check points at a file path, a test exit code, a named critic's
...
4. **Declared default-on-silence.** Every open question the delegate can't resolve states the DEFAULT it will take
   if unanswered, and it keeps working (ref:skill/grimorio.fan-out#part-2--stay-reachable-report-back-without-parking). It never blocks on your latency.
5. **A hard failsafe bound — and a LOUD failure on hitting it.** Every mature framework pairs a "natural" stop
   ...
   ref:skill/grimorio.fail-fast) — a failsafe-terminated run is a declared failure, never a done-ish report.
...
7. **Finish-synchronously by DEFAULT; background is a considered trade, not a forbidden one.** Foreground
   ...
   session has armed that watch this session (ref:skill/grimorio.conduct/project.main-loop-only.md rule 8) — the brief
   ...
   optional faster child-self-report path: ref:skill/grimorio.flow-delegation/project.nested-background-trade.md. **This item
   covers backgrounding the delegate's own CHILDREN only — for the delegate's own Bash/PowerShell/`Monitor`
   calls, ref:skill/grimorio.conduct#spawning-an-agent rules 9b-9c state the general prohibition and the
   correct foreground technique.**

> **`ref:skill/grimorio.fan-out`'s own N-child synthesis panel is a NARROWER, still-unreversed case of the same
> reversal** (CEO, ref:skill/grimorio.conduct#spawning-an-agent rule 8) —
> `ref:skill/grimorio.flow-delegation/project.nested-background-trade.md` states the boundary and why; read it before briefing
...
7. **Accumulate the principal's OPEN questions for the wrap-up.** In autonomous/all-night mode the principal is
   ...
   (ref:skill/grimorio.report-design), in the autonomous-mode wrap-up — the default it took, and the decision still open.
...
spawn delegate "policy-sweep-b1":
  brief = flow-brief(
    ...
    no-park:    "default: foreground (run_in_background:false) for every sub-agent; WHEN real parallelism is
                ...
                top-level session's watch (ARMED this session, per ref:skill/grimorio.conduct/project.main-loop-only.md
                rule 8 — never automatic) to rescue a parked child, never a wake you invent yourself; NEVER end
                your turn on a backgrounded tool call you depend on — poll service-readiness in a foreground
                bounded loop, run a long command you act on in the foreground"
  )
...
| No completion checks / "finish the task" | The delegate self-decides done, and self-only stopping degrades quality (Huang et al., ICLR 2024) |
...
-> Plumbing (per-delegate id, notes folder, watcher, no-park): ref:skill/grimorio.fan-out#part-2--stay-reachable-report-back-without-parking. Bibliography + all citations:
   this project's own research bibliography. The two operating
   modes live in this skill's own "The two operating modes — NORMAL and AUTONOMOUS" section above; the full CEO
   ruling + activation logic: this project's own research bibliography.
   Intent fidelity in briefs: ref:skill/grimorio.agent-writing#2-identity-paragraph (PRINCIPAL-INTENT FIDELITY). Tier the spawn: ref:skill/grimorio.agent-tiers#how-to-apply-it-the-mechanics.
...
`import:skill/grimorio.developer-memory/project.build-protocol.md#who-commits-depends-on-whether-you-are-worktree-isolated` — a
