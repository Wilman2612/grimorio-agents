...
-> Companions: ref:skill/grimorio.fan-out#part-2--stay-reachable-report-back-without-parking (Part 2 — surface a blocker mid-run instead of stalling), ref:skill/grimorio.experiment-method (when the failure must be settled by a controlled run), ref:skill/grimorio.report-design (how to present the result — including the negative one).
