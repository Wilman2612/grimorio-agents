...
measured incident or a verbatim source, offloaded to `./delegation-decision.md`, `./measured-runs.md`, and
`./anti-patterns-and-mechanism.md` under the same rule.
...
**NEVER let the fan-out fleet inherit the caller's expensive model.** Tier each spawn per ref:skill/agent-tiers#the-orchestration-cascade-cost-discipline--enforce-it — grunts
...
- **NEVER make the split mandatory.** Serial is right when a piece depends on the one before it. What IS
  mandatory is that the caller CONSIDERED it and said so — including "this does not split, because X".

-> deeper: ./delegation-decision.md — the measured evidence behind the rules above (the `grimorio.qa` fan-out
...
(ref:skill/fan-out#emit-the-loop-graph-before-you-spawn-or-write-hard-rule-ceo-2026-08-08) already requires you
...
downstream of it. -> deeper: ./delegation-decision.md — the full sourcing for every rule below.
...
2. **Push the judgement-free portion down to Haiku** — one child (the execution shape) or several (the
   mining/volume shape), reviewed after. -> ref:skill/agent-tiers#haiku-as-the-first-option-for-executors--two-sanctioned-shapes-never-a-third-ceo-ruling-2026-08-12
...
capability, perspective, source, and claim below — see ref:skill/fan-out/anti-patterns-and-mechanism.md#anti-patterns-each-caused-a-real-failure-or-will. This forbids splitting ONE
piece across role-isolated AGENTS; it does not forbid
ref:skill/agent-tiers#haiku-the-volume-tier--plan-on-sonnetopus-execute-on-haiku-review-on-sonnet's plan→execute→review pipeline
...
more thorough" is not that argument, priced against ref:skill/agent-tiers#the-scale-task-archetype--tier's own
Haiku→Opus spread.
...
**BEFORE choosing an axis ⟶ check whether part of the task is already fully decided and judgment-free — that part is TIER DELEGATION, not a fan-out piece: route it to a single cheap child per ref:skill/fan-out#not-every-task-is-a-fan-out--the-single-child-shape-ceo-2026-07-30, or down a tiered plan→execute→review pipeline per ref:skill/agent-tiers#haiku-the-volume-tier--plan-on-sonnetopus-execute-on-haiku-review-on-sonnet. Only the JUDGMENT-bearing remainder needs an axis at all.**
...
2. **Spawn one agent:grimorio.scout per piece, in parallel, each in clean context, each its OWN id and workspace.**
   ...
   each per ref:skill/agent-tiers#the-scale-task-archetype--tier** (grunts on the cheap tier; never let the fleet inherit the caller's expensive model —
...
-> deeper: ./measured-runs.md — the three-stage measurement and the partition-unit worked example (REFERENCE,
...
ref:skill/flow-delegation#part-1--the-flow-brief-template-how-you-raise-the-delegate's job already; duplicating
...
ref:skill/fan-out#the-workspace-and-isolation-rules-already-stand-see-hard-rules-above and
ref:skill/working-memory#the-folder already define — invent no new location. A graph is a PLAN, never a signed
...
./measured-runs.md — the full evidence: which agents' own behavior files even carry a pointer to this rule,
...
6. **ALWAYS execute this step as the ladder's own instance of the general graph rule above**
   (ref:skill/fan-out#emit-the-loop-graph-before-you-spawn-or-write-hard-rule-ceo-2026-08-08): for a same-type
...
-> deeper: ./anti-patterns-and-mechanism.md — the model-driven vs code-driven distinction, and the anti-pattern
...
> ref:skill/documentation-memory doc 53).** A direct child of the top loop CAN report with `SendMessage(to:"main")` and
...
> ref:skill/flow-delegation#part-1--the-flow-brief-template-how-you-raise-the-delegate's escape hatch). Never write `to:"main"` into a brief for anything but a DIRECT child of the
> top loop.
...
3. **The caller answers with `SendMessage`** to the child's id — it arrives at the child's next tool round, while it kept working. If the note is a decision only the CEO can make, the caller relays it to him digestibly (ref:skill/report-design) and forwards the answer.
...
confused, just unbounded — see ref:skill/flow-delegation#part-1--the-flow-brief-template-how-you-raise-the-delegate's failsafe-bound requirement, whose absence is what let this
happen.

**And never put a reporting rule in a brief that the agent's behavior file does not carry.** Only
agent:grimorio.delegate carries a milestone rule — ref:skill/flow-delegation#part-1--the-flow-brief-template-how-you-raise-the-delegate's "Notes folder + MILESTONE cadence + a STUCK
...
-> Companion skills: ref:skill/agent-tiers#how-to-apply-it-the-mechanics (which model + effort per stage — Part 1) and ref:skill/flow-delegation#part-1--the-flow-brief-template-how-you-raise-the-delegate (raising ONE
