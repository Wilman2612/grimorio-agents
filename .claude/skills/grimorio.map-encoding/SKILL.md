...
the chosen encoding, why it beat the alternatives, and the traps to avoid. Every claim below was grounded in a
controlled experiment that no longer exists on disk (confirmed 2026-08-04) — there is no paper left to re-open if a
number is in doubt. This skill's own text — the chosen encoding, the measured numbers in "Why it wins" and "Rejected
alternatives" below, and the OPEN items — **is** the self-contained surviving record of that experiment; treat it as
such, not as a summary of something still independently checkable.
...
`arch-decision-levels-and-text.md`. The controlled experiment that produced the numbers above (formats' actual
text, token tables, per-tier results, reproducibility chain) is no longer on disk — this skill's own text, above,
is what survives of it.
