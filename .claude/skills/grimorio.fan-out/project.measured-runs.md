...
Split out of ref:skill/grimorio.fan-out/SKILL.md#part-1--decompose-spawn-in-parallel-synthesize (2026-08-08) under
...
("split by TOPIC into self-contained files; never drop knowledge"). `grimorio.fan-out/SKILL.md` is loaded in FULL by
...
(ref:skill/grimorio.fan-out#split-planning-from-execution--a-temporal-axis-for-pieces-that-only-collide-at-write-time-ceo-2026-08-03
...
- Stage 3 ran UNBOUNDED — its brief named the queue size and gave no stop condition, where stages 1-2 were
  bounded and came back clean. -> this project's own defects ledger → "A fan-out brief named
...
- Self-review checkpoints cannot catch fabrication — a worker that invents its work invents its checks too;
  ...
  boundaries. -> this project's own defects ledger → "Self-review checkpoints answered green
...
- A file whose own CONTENT is ABOUT the transformation being mechanically applied — because it narrates the
  ...
  it out. -> this project's own defects ledger → "AS-IS reference-migration Haiku shards corrupted
...
Split out of ref:skill/grimorio.fan-out#emit-the-loop-graph-before-you-spawn-or-write-hard-rule-ceo-2026-08-08 — the
...
loading `grimorio.fan-out/SKILL.md` whole and finding this section unaided); and the failure has NOT been located at
...
ref:skill/grimorio.agent-selection#loop-vs-graph--grimorios-own-working-methodology-decided-2026-07-28 already forbids
picking an enforcement mechanism by inertia.

---

-> Back to the operative rule this evidence supports: ref:skill/grimorio.fan-out#split-planning-from-execution--a-temporal-axis-for-pieces-that-only-collide-at-write-time-ceo-2026-08-03.
