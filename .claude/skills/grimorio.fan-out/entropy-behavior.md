...
> **Raise each scout per ref:skill/grimorio.flow-delegation#part-1--the-flow-brief-template-how-you-raise-the-delegate.** Give each a flow-brief — its ONE lens as the objective + full
...
> never its self-report. Tier each per ref:skill/grimorio.agent-tiers#the-scale-task-archetype--tier.
...
## Steps
1. **ALWAYS state your own graph before doing anything else: four sequential nodes —
   PLAN/DECOMPOSE-THE-PLAN-UNDER-REVIEW-ALONG-A-PERSPECTIVE-AXIS (entropy itself, per
   ref:skill/grimorio.loop-and-graph#1-decompose-first--general--abstraction--specific-until-a-thing-is-testable,
   reads the target and SCOPES each applicable perspective to THIS one plan/design/decision — never an abstract
   "perspective" left undeclared of what it is a perspective ON) (step 2 below) → SPAWN-THE-PERSPECTIVE-PANEL (step 3 below) →
   CONVERGE-BLINDSPOTS (steps 4-5 below) → DONE (step 6 below) — and no other node anywhere in it.** Entropy
   ITSELF is the graph's first node, never the panel spawn: the fan-out is a CHOICE the graph's SECOND node
   makes because pressure-testing genuinely needs independent, uncontaminated lenses (this file's own Core rules
   already state why), never the default every graph opens with. -> ref:skill/grimorio.loop-and-graph#3-the-graph--who-is-in-it-and-the-branch-rule's
   own "you are the first node" doctrine, applied here rather than re-derived.
2. **Frame the target** — what plan/design/decision is being pressure-tested, its goal, and **who it is actually
   for** (the real end user, not the team).
3. **Fan out the perspective panel — YOU spawn it.** One agent:grimorio.scout per lens (hard-locked, non-recursive —
   it can't spawn, so no runaway), each in clean context so the lenses can't contaminate each other. **Tier each
   per ref:skill/grimorio.agent-tiers#the-scale-task-archetype--tier** (per-lens gathering usually Haiku/Sonnet; the convergence is you). Give each scout ONE lens
   ...
   into the ranked blind-spots (step 5) — the pile of per-lens outputs is not the deliverable; your consensus is.
4. **Bring outside prior-art — and FORCE the "does this already exist?" question (highest-value unknown-unknown).**
...
5. **Ground and rank** — each finding cites its quantifiable rule or concrete prior-art, is tagged with the
   perspective that surfaced it, and is ranked by impact × likelihood. Separate **blocking blind-spot** from
   **worth-considering**.
6. **Hand off** — flag deep investigations for `research`, keepers for `documentation`, fixes for the owning
   agent. You store and decide nothing.

## OUTPUT
- **BEFORE Step 2 ⟶ state your objective and exit condition.** THE OBJECTIVE is the plan/design/decision you
...
- Consolidate nothing to permanent memory — you surface; the owning harness records what settles (refer to ref:skill/grimorio.documentation-memory for archival).
