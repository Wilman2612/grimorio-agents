...
different case, not covered by this sentence — ref:skill/grimorio.agent-tiers#how-to-apply-it-the-mechanics for when it
...
ref:skill/grimorio.agent-tiers#the-orchestration-cascade-cost-discipline--enforce-it → "Why this rule, and not a rule on
...
1. **SURVEY, before planning** — bring back information, find where code lives, read a large volume of text,
   ...
   insufficient, AND the volume is large — never "reading is boring." -> ref:skill/grimorio.agent-tiers#the-pattern-that-makes-it-worth-using
   ("PLAN on Sonnet, EXECUTE on Haiku, REVIEW on Sonnet") and ref:skill/grimorio.agent-tiers#where-it-needs-no-plan-at-all
   ("Infrastructure and log analysis") already document this shape; do not restate either here.
2. **EXECUTION, once the work is already planned** — subject to review by a higher tier. -> ref:skill/grimorio.fan-out#not-every-task-is-a-fan-out--the-single-child-shape-ceo-2026-07-30
...
Haiku Supervision Ladder below (ref:skill/grimorio.agent-tiers#the-haiku-supervision-ladder--reviewable-parallelisable-tasks-ceo-ruling-2026-07-30-translated)
...
The same prose/algorithm twin from ref:skill/grimorio.prompt-writing-quality#form-is-the-latitude-instruction--algorithm-vs-prose-ceo-2026-07-30-translated → "FORM IS THE LATITUDE INSTRUCTION" was re-run on Haiku,
...
- **WHEN an agent's frontmatter disallows the `Agent` tool ⟶ that same frontmatter must never declare
  ...
  **Why this rule, and not a rule on the CALLER passing `model` upward:** measured 2026-08-08 from this
  project's own spawn-invocation log, 584 logged spawns, 194 of which passed an explicit `model`.
...
-> deeper: ./project.reference.md — the full archetype catalog (with effort per task), worked examples for
...
- **A critic's tier is FLOORED at the generator's tier — never lower.** A Sonnet build reviewed by a Haiku critic
  ...
  cost leak ref:skill/grimorio.agent-tiers#critic-integrity--the-one-tiering-rule-you-cannot-cheap-out-on exists to stop. Every critic/gate already declares `sonnet` as its own default
...
- **Reward-hack spot-check:** periodically re-judge a PASSED artifact with a fresh unbiased prompt or a higher
  ...
  prompt). -> full grounding + the missing PLAYING critic: ref:skill/grimorio.ai-game-dev-methodology.
...
- **Agent tool**: omit `model` on a normal spawn — the target agent's own declared tier (frontmatter) already
  ...
  would breach the critic-integrity floor** (ref:skill/grimorio.agent-tiers#critic-integrity--the-one-tiering-rule-you-cannot-cheap-out-on:
  a critic's tier is floored at the generator's tier, never lower) **or undercut the tier the cascade assigns
  that stage's archetype** (ref:skill/grimorio.agent-tiers#the-orchestration-cascade-cost-discipline--enforce-it) — either
  case still needs a named reason, same as an upward pass.
- **Workflow `agent()`: NEVER omit `model` on a call.** A raw Workflow call has no agent-declared default to
  inherit the way an `Agent`-tool spawn does — omitting `model` silently inherits the CALLER's own tier,
  frequently Opus, the main loop's own tier, which is exactly the inherited-Opus fan-out cost blowup this whole
  skill exists to prevent, landing at the volume stage where it hurts most. **ALWAYS pass `opts.model` (and
  `opts.effort`) on every call, tiered by stage** — the archetype catalog above and in ./project.reference.md names the
  tier per task shape; the per-stage assignment IS the explicit tiering decision, not an override of anything.
  **WHEN the work is a genuine multi-scout research/gather pass ⟶ PREFER agent:grimorio.researcher or
  agent:grimorio.entropy over a hand-rolled Workflow** — both already tier their own scouts by design
  (agent:grimorio.scout is hard-locked, but declares `sonnet` as its own default; `researcher` and `entropy`
  each explicitly override it DOWN to Haiku or Sonnet per gather-slice at spawn time, rather than relying on
  that default), so reaching for a raw Workflow re-derives tiering discipline that already exists elsewhere
  and is easy to get wrong under long-session attention decay.

  This is written as a hard rule, not prose, because the prose form of this exact guidance already lived in
  this exact section and did not stop a real Workflow run from inheriting the caller's tier across its whole
  fan-out.
...
-> The triggers, the distinctions from `unblocker` / `entropy`, and how to brief each: ref:skill/grimorio.agent-selection#the-escalation-ladder--five-agents-five-different-distress-signals skill →
...
ref:skill/grimorio.agent-tiers/project.refusal-pattern.md before deciding how to respond.** It carries the triad every such
...
-> This is consulted BY the caller that spawns agents (the main loop and any agent that fans out). Agent
   ...
   see ref:skill/grimorio.agent-writing#the-levels--behavior--general--project--code.

-> BEFORE any tier/grimorio.fan-out/delegate-vs-self spawn call ⟶ also read
   ref:skill/grimorio.agent-tiers/project.experiment-decision-rules.md, the measured consequences of WHERE and HOW an instruction
   actually compels a spawned agent — companion to this file, not restated here.
