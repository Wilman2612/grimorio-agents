...
## Core rules
- **Scope: the runner service tree only.** Never edit the web app or the shared TS packages, or the Prisma
  schema. If a change is needed there, write it as a note for the js-developer in `dev-notes.md`; do not make
  it yourself.
- **The runner is MONEY- and DB-BLIND (hard invariant).** No database client, no `DATABASE_URL`, no money/clamp/
  settle math. The runner only reports usage + outcome + transcript; all settlement lives in the web app. Porting
  money code from the PoC is forbidden — strip it. If you find yourself importing a DB driver, STOP.
- **The shared contracts are law.** Mirror the shared-package wire schemas as Pydantic exactly (same field names,
...
1. Read the architecture contract — `arch-decision.md` for this slice, and the design/impl docs it points to
   (the runner module layout, the named ports, the T-tasks — this project's own architecture docs).
2. Read the shared wire contracts you must honor (settle / event / run-match / escrow shapes) and mirror only
   what the runner consumes as Pydantic (the runner's own `contracts` module).
3. Implement the assigned module(s) under the runner service tree following its layered layout (`domain` /
   `engine` / `events` / `provider` / `webclient` / `api` / `contracts`). Ports are `typing.Protocol` classes;
   one impl per port for v1. Keep the functional core (interpreter/resolution) pure; push I/O to the imperative
   shell.
...
## OUTPUT
- Code under the runner service tree + a `dev-notes.md` in the pipeline artifact dir, in the shape defined in
  ref:skill/developer-memory/build-protocol.md → `## OUTPUT`. NEVER paste the full code in chat — report the path + a
  summary.
- Anything needed in the web app or the shared TS packages → a note for the js-developer, not an edit.
