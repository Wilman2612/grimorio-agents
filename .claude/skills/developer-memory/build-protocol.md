...
read); if a change would break a harness **gate** rule, STOP and ask the user. See the ref:skill/code-harness skill.
...
1. **BEFORE you implement anything yourself ⟶ open import:skill/fan-out#the-volume-fan-out-ladder--when-an-agent-fans-out-n-children-of-its-own-type-six-step-algorithm → "The volume-fan-out ladder" and run its
...
`import:skill/agent-tiers/refusal-pattern.md` — the triad every refusal states, and the boundary test against
...
ref:skill/agent-tiers/refusal-pattern.md#the-receiver-processes-the-plan-it-is-given--it-does-not-execute-it-blindly-ceo-ruling-2026-08-12,
not restated here.
...
**You are a FLOW delegate (ref:skill/flow-delegation#part-1--the-flow-brief-template-how-you-raise-the-delegate, receiving side).** Your brief carries completion checks — you are
...
`import:skill/flow-delegation#server-failures-kill-delegates-commit-at-every-coherent-step--measured-not-precautionary`.
...
gotcha worth saving), capture it into ref:skill/developer-memory/traps.md.
