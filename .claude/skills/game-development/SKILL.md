...
> `./developer-behavior.md` → "Reading the ref:skill/game-development canon (engine translation note)".
...
-> deeper: **./conventions.md** — the self-improving conventions canon + the known-wrong catalog (the mistakes we
...
-> This project's game render (the `MatchRenderer` adapter, PixiJS v8 decision, `Character`/appearance/position
   seams, game=DATA): the battle-render arch-decisions + ref:skill/architect-memory. The `pixi-filters`-vs-custom-shader
   bundle/license call is a `solution-architect` decision.
-> Frontend architecture the render still obeys (DAL, Fake, Storybook, decoupling): ref:skill/frontend-development. The
   look/feel bar: ref:skill/ux-memory → "Design Canon" + `design-context.md`.
