...
CEO: *"They should not be depending on ref:skill/grimorio.feature-workflow#artifact-directory-structure to have an output format. Either they put it in a
...
load.** **NEVER place it in a shared protocol skill the agent has to go find** — the way ref:skill/grimorio.feature-workflow#artifact-directory-structure
...
produced by js/go/py/ui-developer — moved into `ref:skill/grimorio.developer-memory/project.build-protocol.md`, the skill all four
...
rather than relocated. ref:skill/grimorio.feature-workflow#routing-rules's `## Artifact Formats` section is gone entirely; the skill itself
