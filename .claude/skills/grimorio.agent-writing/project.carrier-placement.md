...
never opened.** The obligation was moved into ref:skill/grimorio.flow-delegation/delegate-behavior.md as an executable
...
open it.** ref:skill/grimorio.prompt-reading owed action #3 orders every agent to read its own named behavior file in
...
machine only loop-owners run, the same COST × FREQUENCY argument at ref:skill/grimorio.agent-writing#audit-lenses.

**KNOWN AND DELIBERATELY NOT FIXED: `agent:grimorio.system-keeper` carries the same bibliography-only
`import:skill/grimorio.loop-and-graph`, and its behavior file never names it as a step.** It was left alone because the
