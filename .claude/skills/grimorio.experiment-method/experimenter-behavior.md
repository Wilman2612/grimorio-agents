...
`SKILL.md`; this project's lab paths/index/baseline live in this project's own experiment memory. Load all three.

## The process — run in order, every time

1. **Load method + read-first.** Read `SKILL.md` + this project's own experiment memory. Then read what the project ALREADY has for THIS
   question — the signed design under test (its ref:tmp/ doc / ref:skill/grimorio.po-memory), this project's own experiment index, and any
...
3. **Check the harness can actually meet the method — and FLAG it if it can't.** Before running, confirm the lab
   ...
   current lab does NOT yet (see this project's own experiment memory §lab-limits) — when an axis is missing, you MUST state the resulting
...
9. **Write the DIGESTIBLE COMPANION** `experiments/<slug>/companion.md` — the mandatory plain-language CEO brief.
   Reuse the diagram-first proposal shape from the ref:skill/grimorio.game-design skill (do not invent a new one). Bounded:
   ~1 page, the 2–3 numbers that matter translated out of jargon, what it means + what's open. No `§` notation.
10. **Update this project's own experiment index** — one row (slug, question, verdict, date-run) so "what have we run?" and the
...
- **Auto-invocation has a precision boundary:** you produce a paper only when a real design HYPOTHESIS is being
  settled by a run. A QA smoke run, a one-off sanity `sim lab`, or a build check does NOT trigger a paper.
