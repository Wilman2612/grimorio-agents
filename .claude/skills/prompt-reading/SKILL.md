...
-> Their authoring side, and how each composes with the four openers:
   import:skill/prompt-writing-quality/control-flow-vocabulary.md
...
ref:skill/prompt-writing-quality#an-opener-is-necessary-but-not-sufficient--a-rule-must-also-name-an-action-owed-inside-the-task-main-loop--grimoriosystem-keeper-2026-08-11,
...
1. **BEFORE acting on any instruction anywhere in this corpus ⟶ read import:skill/prompt-reading IN FULL.** It
...
3. **ALWAYS read your own named behavior file IN FULL and execute it exactly, every invocation, and WHEN the
   ...
   file is where everything the agent DOES actually lives. ->
   ref:repo/.claude/GRIMORIO-CHAIN.md#1-the-context-boundary--the-thing-that-is-most-often-gotten-wrong for WHY
   this reaches every sub-agent identically.
4. **BEFORE creating, modifying, or inspecting any file ⟶ walk the upward `harness.md` chain from that file's
   ...
   -> ref:skill/code-harness#the-lookup-protocol-how-you-use-a-harness.
...
ref:skill/reasoning-principles#state-your-objective-and-exit-condition-then-close-verified-or-could-not-hard-rule-ceo-2026-08-11's
...
The honest remaining boundary: the hook's own header exempts six agent types — `cv-ats-screener`,
`cv-recruiter`, `cv-reviser`, `statusline-setup`, `claude-code-guide`, `grimorio.experimenter` — verified to
carry no `Skill` tool at all, so gating them would only ever deny a spawn, never compel one. That exemption list
...
-> The AUTHORING side, a different job and the writer's:
   import:skill/prompt-writing-quality/format-guide.md
