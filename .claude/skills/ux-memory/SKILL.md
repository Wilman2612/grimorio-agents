---
name: ux-memory
description: "Semantic memory for the adversarial UX critic. SKILL.md (general) = universal UX principles, the design canon (Norman, the Laws of UX, Refactoring UI, WCAG) with checkable thresholds, and the Nielsen heuristics evaluation framework, used to tear down rendered UI. For this project's design system and conventions (project/code) read ./project.md and ./design-context.md."
---
...
-> GROUND every finding: "the run button is a 20 px flat text link — Fitts + missing affordance: too small, no
   ...
   is also what the **agent:grimorio.ui-developer builds TO** (via ref:skill/frontend-development) — build to it, don't just critique against it.

-> deeper: **./component-reference.md** — the technified COMPONENT & systems canon (primary-sourced,
...
-> deeper: **./premium-aesthetics.md** — the POC→PREMIUM criteria that survived adversarial
   ...
   is ~20% these criteria + ~80% committed craft/identity (Esports-premium, see ./design-context.md).
...
-> This project's design system (component names, tokens, patterns): read ./project.md and ./design-context.md
