...
in named prior art (see `documentation-memory/docs/37…` PART G — the empirical-lab research); when a rule and the
...
sample — no seeds, no stats, cheap and legible. This is the "agent that PLAYS it" gate from ref:skill/ai-game-dev-methodology,
...
`documentation-memory/docs/37…` PART G. This project's lab + paths + baseline: ./project.md.
