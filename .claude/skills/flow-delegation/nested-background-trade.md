...
| **Nested foreground** (`run_in_background:false`) | Block-and-resume works at any depth. **DOCUMENTED:** the depth limit is CONFIGURABLE via `CLAUDE_CODE_MAX_SUBAGENT_SPAWN_DEPTH`, documented default `3` layers below the main conversation; this repo's `.claude/settings.json` sets it to `5`. **NOT MEASURED:** whether a depth-5 spawn actually succeeds — untested. Session cap 200 subagents. Full sourcing: ref:skill/documentation-memory/docs/53-claude-code-subagent-execution-notification-semantics-referencia.md finding 3. | ✅ Safe to the documented default depth (3) — the correct default way a delegate runs its own children; deeper rests on this repo's configured, untested knob. |
...
rests on this repo's configured, untested knob (ref:skill/documentation-memory/docs/53-claude-code-subagent-execution-notification-semantics-referencia.md finding 3).
...
2. The parent injects that id into each child's brief + the instruction: *"when done, `ToolSearch
   ...
   and looks delivered anyway (two live probes, cite:skill/documentation-memory doc 53 Resolution 3).
...
`ref:skill/fan-out`'s own rule ("NEVER spawn a fan-out child in the background") stands unchanged. That is a
...
`entropy`/`researcher`/`solution-architect` (which spawn nested scouts via `ref:skill/fan-out`'s own protocol)
