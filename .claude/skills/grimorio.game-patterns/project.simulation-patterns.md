...
anywhere in sim logic**. The variable-render half belongs to the render, not here — see ref:skill/grimorio.game-development.
...
4. **Dormant objects still cost.** They burn CPU and thrash cache in the main collection; a separate active-only
   ...
   see ref:skill/grimorio.game-patterns/project.data-oriented-design.md.)
...
- **No iteration over non-deterministically-ordered collections.** (In Go this is the language's single biggest
  determinism trap — see ref:skill/grimorio.golang.)
...
> **Read this against ref:skill/grimorio.game-patterns/project.data-oriented-design.md.** Fabian's existential processing directly contradicts the
