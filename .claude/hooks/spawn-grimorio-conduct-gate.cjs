 ...
 * now names `grimorio-conduct` as the one skill every agent loads first, and `grimorio-conduct` itself, as its
 * own first step, compels loading `prompt-reading` in turn. This file was renamed from
 ...
 *   1. The rule REACHES the agent it governs — CLAUDE.md prohibition 25 (prompt-writing-quality before
 ...
 * types, the platform's own agent listing. None of the six carry the `Skill` tool, so none of them can act
...
// The six agent types measured (see header) to carry no `Skill` tool — gating them would only ever deny,
...
function denyMessage() {
  return (
    ...
    'the first thing every grimorio agent does, and its own first step then compels loading prompt-reading in ' +
    'turn."\n\n' +
