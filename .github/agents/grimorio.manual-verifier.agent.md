...
2. **`ux-spec.md`** — The UX contract. If this file exists, it is **equally authoritative** as the PO brief. Each item in the UX Heuristics Checklist = one additional visual check. Pay special attention to: back/exit navigation on every screen, all 4 data states (loading/empty/error/populated), accessible labels, and mobile layout.
...
**Option C — BLOCKED (hardware/OS features)**
...
```
If the count is **0**, do NOT write a DONE or FAIL report. Write a `BLOCKED` report explaining that no screenshots were captured — the browser tool was unavailable or failed. A report without screenshot evidence is not a valid verification.
