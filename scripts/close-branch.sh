...
#
#   scripts/close-branch.sh [--dry-run] [--no-merge]
#
...
if [ "${nrunnable:-0}" -lt "${closed:-0}" ]; then
  ...
  awk '
    ...
    END { flush() }
  ' "$objfile" >&2
  fail "$(( closed - nrunnable )) ticked check(s) carry no runnable VERIFY command of their own. A check only a human can confirm is how a practice gets adopted in words and never used."
fi
...
while IFS= read -r cmd; do
  ...
  if ! out=$(bash -c "$cmd" 2>&1); then
    echo "$out" | tail -20 >&2
    fail "a check's VERIFY failed: $cmd"
  fi
done <<< "$verifies"
...
Gate: scripts/close-branch.sh" || fail "the close-out commit failed."
