...
# WHICH objective governs a branch is decided in exactly one place — scripts/objective-lib.sh — and the
...
#
#   scripts/objective-current.sh            # header + the full objective file
#   scripts/objective-current.sh --compact  # header + statement + open checks + out-of-scope paths
#   scripts/objective-current.sh --path     # just the resolved file path
#
...
if [ "$origin" = "own" ]; then
  ...
  echo "  narrower objective of its own: scripts/open-branch.sh --here \"<one sentence>\")"
fi
echo ""
